...
function ProductionPage() {
  ...
  const handleCompleteOrder = async (orderId, event) => {
        ...
        {orders.length > 0 ? (
          orders.map(order => (
            ...
            <Link to={`/order/${order.id}`} key={order.id} className="block group">
              <div className="bg-white p-4 rounded-lg shadow-md flex justify-between items-center transition-all duration-300 group-hover:shadow-lg group-hover:border-green-500 border border-transparent">
                <div>
                ...
                <button 
                  ...
                  className="px-4 py-2 bg-green-500 text-white font-bold rounded-md hover:bg-green-600 z-10"
                >
                  Selesaikan Produksi
                </button>
              </div>
